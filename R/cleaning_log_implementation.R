...
df_cleaned_data <- df_cleaning_step|> 
    select(-`...1198`) |> 
    mutate(across(starts_with("women_unsafe_location_yes/"), 
                  .fns = ~case_when(!is.na(women_unsafe_location_yes)  & str_detect(string = women_unsafe_location_yes, 
                                                                                    pattern = str_extract(string = cur_column(), pattern = "[\\w]+.$")) ~ TRUE,
                                    !is.na(women_unsafe_location_yes) & !str_detect(string = women_unsafe_location_yes, 
